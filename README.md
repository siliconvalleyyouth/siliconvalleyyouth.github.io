...
Teacher/officer card images and shared profile fields are centralized through `profiles.js` and `SVYProfiles`. The backend `npm run backfill:profile-descriptions` script fills missing `profiles.js` descriptions from the latest non-empty teacher bio, and `profiles.js` overlays newer backend profile metadata when available. Officer history should list only officer membership, role, and role-specific copy; do not duplicate image/profile data there.

`teachers.js` should only store year membership by `profileId`. Names, image filenames, descriptions, and profile URLs come from `profiles.js`.
