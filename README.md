...
Teacher/officer card images and shared profile fields are centralized through `teachers.js` and `SVYProfiles`. `profile-descriptions.js` is generated from backend class sheets/archives with `npm run backfill:profile-descriptions` in the backend repo; it fills missing profile descriptions from the latest non-empty teacher bio and overlays newer backend profile metadata when available. Officer history should list only officer membership, role, and role-specific copy; do not duplicate image/profile data there.
